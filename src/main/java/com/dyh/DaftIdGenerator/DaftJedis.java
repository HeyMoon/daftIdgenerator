...
import com.dyh.DaftIdGenerator.exception.InvalidServerFormatException;
 ...
 *
 * Implementation of the  Redis interface for Jedis.
 */
public class DaftJedis implements Redis {
  private static final Pattern SERVER_FORMAT = Pattern.compile("^([^:]+):([0-9]+)$");

  private final JedisPool jedisPool;

  /**
   * Create an instance of DaftJedis from a host and port string of the format "server:port".
   *
   * @param hostAndPort A host and port string for a Redis instance to use for ID generation, of the format "host:port".
   */
  public DaftJedis(final String hostAndPort) {
  ...
  /**
   * Create an instance of DaftJedis from an existing JedisPool instance.
   *
   * @param jedisPool An existing JedisPool instance you have configured that can be used for the ID generation.
   */
  public DaftJedis(final JedisPool jedisPool) {
   ...
   */
  @Override
  public Optional<RedisResponse> evalLuaScript(final String luaScriptSha, final List<String> arguments) {
    return withJedis(jedis -> {
      ...
      try {
        ...
        return Optional.of(new RedisResponse(results));
