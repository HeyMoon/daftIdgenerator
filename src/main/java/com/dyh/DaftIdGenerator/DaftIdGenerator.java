...
import com.dyh.DaftIdGenerator.core.IdGenerator;
import com.dyh.DaftIdGenerator.exception.InvalidBatchSizeException;
import com.dyh.DaftIdGenerator.exception.InvalidLogicalShardIdException;
import com.dyh.DaftIdGenerator.exception.LuaScriptFailedToLoadException;
import com.dyh.DaftIdGenerator.redis.Redis;
import com.dyh.DaftIdGenerator.redis.RedisResponse;
 ...
 */
public class DaftIdGenerator implements IdGenerator{
  private static final Logger logger = LoggerFactory.getLogger(DaftIdGenerator.class);
   ...
   */
  public DaftIdGenerator(final RoundRobinRedisPool roundRobinRedisPool) {
   ...
   */
  public DaftIdGenerator(final RoundRobinRedisPool roundRobinRedisPool, final int maximumAttempts) {
   ...
   */
  public DaftIdGenerator(final RoundRobinRedisPool roundRobinRedisPool, final int maximumAttempts, final long customEpoch) {
  ...
  private Optional<List<Id>> generateIdsUsingRedis(final Redis redis, final long batchSize) {
    Optional<RedisResponse> optionalIcicleRedisResponse = executeOrLoadLuaScript(redis, batchSize);

    if (!optionalIcicleRedisResponse.isPresent()) {
      return Optional.empty();
    }

    RedisResponse redisResponse = optionalIcicleRedisResponse.get();
    ...
    // these values.
    long timestamp = (redisResponse.getTimeSeconds() * ONE_SECOND_IN_MILLIS)
        + (redisResponse.getTimeMicroseconds() / ONE_MILLI_IN_MICRO_SECS);
    long shiftedTimestamp = (timestamp - customEpoch) << TIMESTAMP_SHIFT;

    long logicalShardId = redisResponse.getLogicalShardId();
    validateLogicalShardId(logicalShardId);
    long shiftedLogicalShardId = logicalShardId << LOGICAL_SHARD_ID_SHIFT;

    List<Id> ids = new ArrayList<>();
    for (long sequence = redisResponse.getStartSequence(); sequence <= redisResponse.getEndSequence(); sequence ++) {
   ...
   */
  private Optional<RedisResponse> executeOrLoadLuaScript(final Redis redis, final long batchSize) {
    Optional<RedisResponse> response = executeLuaScript(redis, batchSize);
   ...
   */
  private Optional<RedisResponse> executeLuaScript(final Redis redis, final long batchSize) {
