local lock_key = 'daft-generator-lock'
local sequence_key = 'daft-generator-sequence'
local logical_shard_id_key = 'daft-generator-logical-shard-id'

local max_sequence = tonumber(KEYS[1])
local min_logical_shard_id = tonumber(KEYS[2])
local max_logical_shard_id = tonumber(KEYS[3])
local num_ids = tonumber(KEYS[4])--batchSize

if redis.call('EXISTS', lock_key) == 1 then
  redis.log(redis.LOG_NOTICE, 'Daft: Cannot generate ID, waiting for lock to expire.')
  return redis.error_reply('Daft: Cannot generate ID, waiting for lock to expire.')
end
...
if end_sequence >= max_sequence then
  ...
  --]]
  redis.log(redis.LOG_NOTICE, 'Daft: Rolling sequence back to the start, locking for 1ms.')
